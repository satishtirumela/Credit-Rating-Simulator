...
## G. Post-v3.0 runtime remediation — rationale, citation, and Firestore fixes

**12 August 2026 · Antigravity session**

A separate remediation pass, later and structurally different from A–F: these are runtime code defects found in the live application after v3.0 shipped, not document edits, discovered while verifying the rationale/citation pipeline end to end. Not counted in the "45 findings" total above and not merged into sections A–F's tables.

This is the log's first section to reference git commits. `Where` still means the code location of the defect, as in sections B–D. `Commit` is new to this section only — the point-in-time record of the fix — and is not a pre-existing convention of this log.

### G.1 Live-path bugs — wrong output on the real, frontend-reachable `/approve` endpoint

| Ref | Defect | Resolution | Where | Commit |
| --- | --- | --- | --- | --- |
| **L1** | `draft_rationale()` silently substituted hardcoded placeholder numbers (`average_dscr=1.52`, `dsra_months=6.0`, `installed_capacity_mw_ac=150`, `contracted_revenue_share=0.88`) whenever the scoring engine returned `None` for these fields. Nothing in the output distinguished a fabricated figure from a genuine one. | All fabricated fallbacks removed. A missing field's clause is now omitted from the narrative and logged as a plain-language entry in a newly-functional `uncited_claims` list (previously hardcoded to `[]`, populated by no code path). Two fallback branches (`min_dscr`, `distance_to_band_edge`) confirmed structurally unreachable on any scored result and left untouched. | `app/rationale/draft.py` — `draft_rationale()` | `0803937` |
| **L2** | `run_approved_assessment_pipeline` discarded `draft_rationale()`'s own technology-branched citations and substituted a fixed, technology-agnostic 3-item catalog, whose third entry cited a wind-specific methodology document regardless of the project's actual technology — solar and hybrid projects cited a wind PDF. Confirmed reachable from the live, frontend-triggered `POST /api/projects/{id}/approve` flow, not a dead code path. | Pipeline now verifies `draft_rationale()`'s own citations in place via `verify_methodology_citations(citations)`, never substitutes a separate set. Static catalog (`VERBATIM_CITATIONS_CATALOG`) removed. | `app/pipeline.py` — `run_approved_assessment_pipeline()`; `app/grounding.py` | `af542ef` |
| **L3** | `qa_review_assessment()`'s coherence check was `len(citations) == 3` — a tautology verifying only the old static catalog's fixed size, not real content coherence. Would have been silently wrong by construction once citation count began varying by technology (2–3 items). | Replaced with a content-based check: non-empty citations on a rated band (empty required for Not Rated), every citation carrying `claim`, `source_document`, and `grounding_status == VERIFIED_VERBATIM`. | `app/pipeline.py` — `qa_review_assessment()` | `af542ef` |

### G.2 Verification / tooling

| Ref | Defect | Resolution | Where | Commit |
| --- | --- | --- | --- | --- |
| **V1** | Module/function docstrings stated rationale drafting is done "via Gemini." `draft_rationale()` is deterministic Python string templating; Gemini is used elsewhere (`app/extraction.py`) only for document extraction. Documentation-only inaccuracy — no functional effect. | Docstrings corrected. Not a functional change. | `app/pipeline.py` (module + function docstrings) | `0803937` |
| **V2** | No field distinguished a narrative sentence naming a specific Tier 2 (sector-context) document from the Tier-1-only `citations` list. | Added `sector_context_sources`, tracking which narrative block names which Tier 2 document (currently only the `TECH_WIND` branch). Structurally separate from `citations`; not merged, aliased, or cross-populated. | `app/rationale/draft.py` — `draft_rationale()`, `get_grounded_methodology_citations()` | `0803937` |
| **V3** | Once real verbatim verification was applied, 4 of 5 hardcoded citation claims in `get_grounded_methodology_citations()` were found non-verbatim — paraphrased, stitched from non-adjacent sentences, or absent from the source PDF entirely. Only the Crisil claim was genuine. | All 4 (CARE_Criteria, CARE_Wind, CARE_Solar, ICRA_Hybrid) replaced with real verbatim excerpts, independently self-verified against extracted PDF text before selection, sourced with sign-off per branch. | `app/rationale/draft.py` — `get_grounded_methodology_citations()`; `app/grounding.py` — `verify_methodology_citations()` | `ded451f` |
| **V4** | The `TECH_HYBRID` narrative claim ("generation complementarity / combined tariff stability") had no support anywhere in its cited source, the ICRA hybrid methodology document. The document's only genuine hybrid content concerns storage-pairing for round-the-clock supply. | `resource_risk_sentence`'s `TECH_HYBRID` branch rewritten to match what the source document actually supports. | `app/rationale/draft.py` — `resource_risk_sentence` (`TECH_HYBRID` branch) | `ded451f` |
| **V5** | Two Firestore documents (`TP-4`, `NEG-CAP-1`) carried a still-older stale pattern predating even L2's fix — a static-catalog citation shape (a `"filename"` key not produced by any current code path). 11 real approved projects total, not the originally-identified 10, found carrying at least one of: fabricated citations, the pre-L2 static-catalog citations, the fabricated numeric-default signature (L1), or stale pre-V4 hybrid narrative text. Separately, a diagnostic script run to verify L2 wrote 9 real test-artifact documents (`VERIFY-CITATIONS-*`) into production Firestore, an unrelated side effect (see V6). | Pre-rescore snapshots exported for all 11 affected projects (local, gitignored, under `diagnostics/pre_rescore_snapshots/`). The 9 stray test-artifact documents deleted after confirming zero repo references. All 11 real projects re-scored via the corrected pipeline; zero `final_band` changes confirmed on every project — this was purely a rationale/citation-layer defect, never a scoring-layer one. | Cloud Firestore `projects/{TP-1, TP-2, TP-2-Mid-Wind, TP-3, TP-4, TP1 Suryagarh Solar, TP2 Vayu Wind, TP3 Trishul Hybrid, TP4 Vindhya Hybrid, SolairePower, NEG-CAP-1}`, via `app/pipeline.py::run_approved_assessment_pipeline()` | n/a — direct Firestore data operations, no code change |
| **V6** | No test/diagnostic isolation existed anywhere in the codebase. Credential-check logic, independently duplicated across 5 call sites in 2 files, always resolved to real Firestore whenever valid credentials existed on disk — the direct cause of V5's stray writes. | Centralized into a single `get_firestore_client_or_none()`, used by all 5 call sites. Added a guard, checked before any real Firestore call: `PYTEST_CURRENT_TEST` (automatic under pytest, zero configuration) or `CRS_FORCE_LOCAL_FIRESTORE` (explicit opt-in for ad-hoc scripts) routes both reads and writes to the local JSON fallback — never a mixed state. Visible log line on first activation. Production (unguarded) behavior confirmed unchanged by default. | `app/firestore.py` — `get_firestore_client_or_none()`; `app/extraction.py` — `save_to_firestore()` | `277d77a` |

**Note — unrelated stray file, found during commit-splitting, not part of this remediation chain:** `scratch/diff_frontend_copies.py` was found untracked while separating this session's commits. It is unrelated to any of G.1/G.2's fixes. Left as-is, untracked, in the working tree. Keep-and-gitignore vs. delete is an open decision, not made here.

### G.3 Verification performed

| Check | Result |
| --- | --- |
| Raw diffs (`git diff`) reviewed line-by-line before every fix was accepted | `app/rationale/draft.py`, `app/pipeline.py`, `app/grounding.py`, `app/firestore.py`, `app/extraction.py` — all pasted and reviewed in full at each stage, not summarized |
| `verify_methodology_citations()` run against real corpus PDFs | All 4 replacement claims (V3) self-verified — `normalize_pdf_text()` + exact substring match — against actual extracted PDF text before being written into `draft.py`, not asserted by inspection |
| Full pipeline re-run, all technology branches | `TECH_WIND`, `TECH_SOLAR`, `TECH_HYBRID`, and the untyped/`else` branch all return `grounding_status: VERIFIED_VERBATIM` on every citation post-fix; 0 `RuntimeError`s |
| `run_approved_assessment_pipeline` on all 9 reference projects + all 11 real Firestore projects | 0 `RuntimeError`s; 0 `final_band` changes on the 11 re-scored real projects |
| Full `pytest -v` suite, real output pasted at every stage | 60/62 passing post-commit-split sanity check; the 2 failures are the same pre-existing, environment-only `ERR_CONNECTION_REFUSED` cases (no dev server running), confirmed unrelated earlier via baseline `git stash` comparison and confirmed passing when a dev server is running |
| Firestore re-reads independent of in-process return values | After re-scoring, all 11 projects re-fetched fresh from Firestore (not the pipeline call's in-memory result) to confirm `final_band`, `citations`, and `grounding_status` actually persisted as expected |
| Guard (V6) verified on both trigger paths | `PYTEST_CURRENT_TEST`: confirmed via real pytest run, log line observed. `CRS_FORCE_LOCAL_FIRESTORE`: confirmed via standalone script, end-to-end through `approve_project_document` → `get_project_document` → `run_approved_assessment_pipeline`, then independently confirmed via a real Firestore read that no document was created in production |

### G.4 Schema-conformance gate follow-on — TP-7 structural exception and a write-path merge-corruption bug

**13 August 2026 · Antigravity session**

A follow-on pass after G.1–G.3 shipped: a schema-validation gate was added to `approve_project_document()` (`9a9a108`), the 9 canonical reference fixtures and 4 real Firestore projects were regenerated against it, and 2 stray/duplicate documents were removed. Two findings from that work:

| Ref | Defect | Resolution | Where | Commit |
| --- | --- | --- | --- | --- |
| **T1** | `TP-7`'s reference fixture cannot be represented as a schema-conformant `approved_data` document: its critical-null test scenario requires `technology_type` to be absent/null, but `technology_type` is `required` and non-nullable in `schemas/project.schema.json`, and the schema-validation gate rejects any payload missing it. No conversion of the fixture can satisfy both constraints at once. | Documented as a permanent, by-design exception, not an oversight. `TP-7`'s only valid representation going forward is as engine-level test input via `score_project()` directly (`tests/test_engine.py`), never through the real `/approve` path. Its live Firestore document (`TP-7`) is intentionally left in its pre-gate, narrative-key-contaminated state — regenerating it through the gate is structurally impossible, not merely undone. | `schemas/project.schema.json` (`technology_type`); `app/firestore.py` — `approve_project_document()` gate | `9a9a108` (gate); no separate code change for T1 itself |
| **M1** | While regenerating the 12 documents above, live re-fetch after write showed old contamination surviving underneath the new clean fields. Root cause: `approve_project_document()` wrote via `doc_ref.set(update_payload, merge=True)`, and Firestore's `merge=True` recursively merges nested map fields (`approved_data`, `field_provenance`) rather than replacing them — any key present in the old document but absent from the new payload silently persisted. Confirmed by a direct test against a disposable Firestore document, not assumed from SDK documentation. Silently defeated the first regeneration attempt on all 9 pre-existing documents — only the 2 brand-new ones (`TP-5`, `TP-6`) came out clean, since they had nothing to merge against. | Existence-check branch added: `doc_ref.update(update_payload)` for a document that already exists (replaces named fields, including nested maps, wholesale; leaves sibling fields like `score`/`scored_at`/`extracted_data` untouched), `doc_ref.set(update_payload)` for a brand-new document. Verified via live re-fetch (not the submitted payload or return value) on a reproduction of the exact bug scenario, plus a first-approval-still-works check, before the 12-document regeneration was retried and succeeded. | `app/firestore.py` — `approve_project_document()` | `f887609` |

**Closing note — `SolairePower` final disposition:** `SolairePower` was quarantined during this cleanup for malformed, non-schema `approved_data` — flattened summary-label keys inconsistent with real UI/extraction output, origin undetermined. Satish reviewed and decided to discard it permanently rather than investigate its origin or reconstruct it. The pre-removal snapshot (`diagnostics/pre_regeneration_snapshots/SolairePower.json`) has been deleted; no further record of this project's data is retained.
