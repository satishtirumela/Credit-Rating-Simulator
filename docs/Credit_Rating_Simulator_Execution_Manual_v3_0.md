...
- **Merchant adjustment** — merchant exposure of 24.99% and of 25.01% on identical cash flows. The second must apply the +0.20x threshold shift.
...
- **Before loading anything, apply the ingestion contract below.** **The corpus is MIXED-FORMAT as remediated.** Of the 41 files carrying a .pdf extension, **37 are ZIP archives** containing one JPEG and one OCR text file per page plus a JSON index, and **4 are genuine PDFs**. *v2.0 of this manual stated that all 41 were ZIP archives and that none was a PDF. That was true of the corpus as originally assembled, but the four files renamed during remediation were re-supplied in PDF form, so a pipeline built on the v2.0 contract now fails on exactly those four — and fails in the same silent, reads-as-corruption way the contract was written to prevent.*

- **Dispatch on the manifest, never on the extension.** The `container` column of the **Reference Corpus Manifest v3.0** declares `ZIP` or `PDF` per file, and the `payload_basis` column records how its content hash was computed. Read the manifest first; branch on `container`; do not sniff, guess, or assume. Run `corpus_manifest.py --verify` before the grounding step: it fails loudly if any file's container or payload hash disagrees with the manifest.

- Instruct Antigravity on all six rules below for the ZIP branch, because four of the six fail *silently* rather than raising:
...
- Instruct Antigravity: ‘Before showing results, run the twelve validation rules at Core Rating Criteria v3.0 Section 10.1, plus checks for AI statements without a citation, and flag anything failing.’ The twelve rules cover: average DSCR not below minimum DSCR; PLCR not below LLCR; offtaker revenue shares summing to 100% ± 1pp; sub-scores summing to the raw score; no block or total exceeding its maximum; the two CFADS routes agreeing within 2%; encumbered DSRA not exceeding total DSRA; the incoherent combination of minimum DSCR below 1.00x with PLCR at or above 2.00x; nil or negative tangible net worth; a merchant adjustment applied wherever merchant exposure exceeds 25%; an operating project’s COD not later than the calculation date; and any stale offtaker rating or ALMM parameter.
