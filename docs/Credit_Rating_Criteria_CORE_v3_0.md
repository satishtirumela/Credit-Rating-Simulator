...
**The corpus is mixed-format.** As remediated, 37 of the 41 files are ZIP containers of per-page JPEG and OCR text, and 4 are genuine PDFs. Ingestion code must **dispatch on the manifest's `container` column** and must not assume either format. The Reference Corpus Hygiene and Ingestion Specification v3.0 states the contract for both.
